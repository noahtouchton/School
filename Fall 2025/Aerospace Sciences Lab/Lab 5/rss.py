...
def build_Cp_by_angle_P_over_q(all_data):
    """
    Compute Cp_i = P_i / q for every angle and every 'Port i' column.
    Returns: Cp_by_angle[angle]['Cp i'] -> Data (mean value and RSS uncertainty)
    Also safe to call multiple times; it does not modify all_data in-place.
    """
    Cp_by_angle = {}

    for ang, cols in all_data.items():
        # find q column
        q_key = next((k for k in cols.keys() if k.strip().lower() == "q"), None)
        if q_key is None:
            # No q in this sheet -> skip or raise
            # raise KeyError(f"No 'q' column found for angle {ang}")
            continue
        qD = cols[q_key]  # Data object for q (mean, u)

        angle_out = {}
        for k, d in cols.items():
            # match columns named exactly "Port N"
            if not re.match(r"^Port\s+\d+$", k.strip(), flags=re.IGNORECASE):
                continue

            i = int(k.split()[-1])  # port index

            # Build temporary Data with symbols matching the formula Cp = P / q
            P_i = Data(name=f"P{i}", var=sp.Symbol("P"), value=d.value, uncertainty=d.uncertainty)
            q_local = Data(name="q", var=sp.Symbol("q"), value=qD.value, uncertainty=qD.uncertainty)

            Cp_i = RSS(name=f"Cp {i}", function="Cp = P / q", data_list=[P_i, q_local])
            Cp_i.var = sp.Symbol(f"Cp{i}")  # nice symbol
            angle_out[f"Cp {i}"] = Cp_i

        Cp_by_angle[ang] = angle_out
