...
// ---------------------- Tool Head Motor Pin Assignments ----------------------
const uint8_t TOOL_HEAD_STEP_PIN = 13;
const uint8_t TOOL_HEAD_DIR_PIN = 12;
const uint8_t TOOL_HEAD_ENABLE_PIN = -1;
const uint8_t TOOL_HEAD_LIMIT_SWITCH_PIN = 8;

// ---------------------- Gantry Motor Pin Assignments ----------------------
...
// ---------------------- Motion Tuning ----------------------
// Tool head motion
const float TOOL_HEAD_MAX_SPEED = 1000.0f;  // steps per second
const float TOOL_HEAD_ACCEL     = 500.0f;   // steps per second^2
const float TOOL_HEAD_CALIB_SPEED = 500.0f; // calibration speed
...
// Homing motion
const float HOMING_SPEED     = 600.0f;   // approach speed toward the switch
...
// ---------------------- Stepper Objects ----------------------
AccelStepper motorToolHead(AccelStepper::DRIVER, TOOL_HEAD_STEP_PIN, TOOL_HEAD_DIR_PIN);
...
long toolHeadMaxSteps = 0L;  // Will be set during tool head calibration
...
void setup() {
  ...
  Serial.println();
  Serial.println(F("Desktop Soil Sampler Full System Calibration"));
  Serial.println(F("Initializing..."));

  // Configure tool head limit switch
  pinMode(TOOL_HEAD_LIMIT_SWITCH_PIN, LIMIT_ACTIVE_LOW ? INPUT_PULLUP : INPUT);
  ...
  // Configure tool head stepper
  motorToolHead.setMaxSpeed(TOOL_HEAD_MAX_SPEED);
  motorToolHead.setAcceleration(TOOL_HEAD_ACCEL);

  // Configure gantry steppers for general travel.
  ...
  // Step 1: Calibrate tool head first
  Serial.println();
  Serial.println(F("========================================"));
  Serial.println(F("Step 1: Tool Head Calibration"));
  Serial.println(F("========================================"));
  calibrateToolHead();
  Serial.println(F("Tool head calibration complete."));
  
  delay(1000); // Brief pause between calibrations

  // Step 2: Home gantry system
  Serial.println();
  Serial.println(F("========================================"));
  Serial.println(F("Step 2: Gantry Homing"));
  Serial.println(F("========================================"));
  ...
  // Step 3: Calibrate gantry X and Y axes
  Serial.println();
  Serial.println(F("========================================"));
  Serial.println(F("Step 3: Gantry Calibration"));
  Serial.println(F("========================================"));
  Serial.println(F("Starting calibration of max limits..."));
  calibrateMaxLimits();
  Serial.println(F("Gantry calibration complete."));
  
  Serial.println();
  Serial.println(F("========================================"));
  Serial.println(F("All Calibrations Complete!"));
  Serial.println(F("========================================"));
  Serial.print(F("Tool head travel: "));
  Serial.print(toolHeadMaxSteps);
...
// ---------------------- Tool Head Functions ----------------------
bool isToolHeadLimitPressed(uint8_t pin) {
  const int state = digitalRead(pin);
  return LIMIT_ACTIVE_LOW ? (state == LOW) : (state == HIGH);
}

void calibrateToolHead() {
  Serial.println(F("Starting tool head calibration..."));
  Serial.println(F("Moving away from limit switch first..."));

  // First, move away from limit switch if it's already pressed
  motorToolHead.setCurrentPosition(0);
  motorToolHead.setSpeed(TOOL_HEAD_CALIB_SPEED); // Move away from limit switchA
  unsigned long timeout = millis() + 5000; // 5 second timeout
  while (isToolHeadLimitPressed(TOOL_HEAD_LIMIT_SWITCH_PIN) && millis() < timeout) {
    motorToolHead.runSpeed();
  }
  motorToolHead.setSpeed(0);
  delay(100); // Brief pause
  
  // Reset position counter
  motorToolHead.setCurrentPosition(0);
  
  Serial.println(F("Moving forward until limit switch is hit..."));
  
  // Move forward until limit switch is pressed
  motorToolHead.setSpeed(-TOOL_HEAD_CALIB_SPEED); // Move toward limit switch
  timeout = millis() + 60000; // 60 second timeout for safety
  while (!isToolHeadLimitPressed(TOOL_HEAD_LIMIT_SWITCH_PIN) && millis() < timeout) {
    motorToolHead.runSpeed(); // Run at constant speed
  }
  
  // Stop the motor immediately when limit switch is hit
  motorToolHead.setSpeed(0);
  motorToolHead.stop(); // Ensure motor stops
  
  // Get the number of steps taken (this is the total travel distance)
  toolHeadMaxSteps = abs(motorToolHead.currentPosition());
  
  Serial.print(F("Tool head calibration complete! Total travel distance: "));
  Serial.print(toolHeadMaxSteps);
  Serial.println(F(" steps"));
...
void homeAxisX() {
  ...
  motorA.setMaxSpeed(absf(speedA));
  motorB.setMaxSpeed(absf(speedB));
...
void calibrateMaxLimits() {
  ...
  moveCoreXYRelative(-currentX(), -currentY(), TRAVEL_MAX_SPEED, TRAVEL_ACCEL);
