...
const float TRAVEL_MAX_SPEED = 1400.0f;  // steps per second
const float TRAVEL_ACCEL     = 500.0f;   // steps per second^2

// Homing motion
const float HOMING_SPEED     = 1400.0f;   // approach speed toward the switch
const float HOMING_ACCEL     = 500.0f;   // acceleration during homing
...
// Offset from home (0,0) to first sample position: X=3cm (30mm), Y=10cm (100mm)
// 30mm * 5 steps/mm = 150 steps, 100mm * 5 steps/mm = 500 steps
long firstSampleBaseXSteps = 50;  // Camera XY for sample 0 (absolute steps)
long firstSampleBaseYSteps = 500;
// Offset from first sample position to first deposit grid position: X=10cm (100mm), Y=40cm (400mm)
// 100mm * 5 steps/mm = 500 steps, 400mm * 5 steps/mm = 2000 steps
long firstDepositGridXSteps = 0L;  // X position (in steps) of deposit grid position 1 (150 + 500)
long firstDepositGridYSteps = 7000;  // Y position (in steps) of deposit grid position 1 (500 + 2000)
...
void setup() {
  ...
  // Note: Edit the calibration variables above (lines 116-120) to set values directly
  // totalSamples, firstSampleBaseXSteps, firstSampleBaseYSteps, 
  // firstDepositGridXSteps, and firstDepositGridYSteps can be set in code
...
void loop() {
  ...
  for (int pos = 1; pos <= numPositions; pos++) {
    ...
    // Store current position for returning later
    long originalXSteps = currentX();
    long originalYSteps = currentY();
    
    // Hover over original grid position for 3 seconds
    Serial.println(F("Hovering over original grid position for 3 seconds..."));
    delay(3000);
    ...
    // Hover over deposit grid position for 3 seconds
    Serial.println(F("Hovering over deposit grid position for 3 seconds..."));
    delay(3000);
    
    // Return to original sample grid position
    Serial.println(F("Returning to original sample grid position..."));
